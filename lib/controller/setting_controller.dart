...
class SettingController {
  ...
  void changePassword(
      var oldPassword, var newPassword, var confirmNewPassword) async {
    if (newPassword == confirmNewPassword &&
        await validateEmailAndPassword(oldPassword)) {
      //Pass in the password to updatePassword.
      _user.updatePassword(newPassword).then((_) {}).catchError((error) {
  ...
  void changeEmail(var oldEmail, var newEmail, var password) async {
    if (oldEmail != newEmail && await validateEmailAndPassword(password)) {
      //Pass in the password to updatePassword.
      _user.updateEmail(newEmail).then((_) {}).catchError((error) {
        //This might happen, when the wrong password is in, the user isn't found, or if the user hasn't logged in recently.
      });
